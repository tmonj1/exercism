...
def check_phone_number:
  if test("\\(\\+\\d{2}\\) \\d{3}-\\d{3}-\\d{3}") then "Thanks! Your phone number is OK." else "Oops, it seems like I can't reach out to \(.)." end
;
...
def nice_to_meet_you:
  capture("my name is (?<name>[\\w-]+)"; "i") | "Nice to meet you, \(.name)"
;
